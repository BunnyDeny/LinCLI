...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
	if (huart->Instance == USART3) {
		cli_in_push((_u8 *)rx_buffer3, Size);
	}
}
...
int main(void)
{
	...
	MX_USART3_UART_Init();
	/* USER CODE BEGIN 2 */
	HAL_UARTEx_ReceiveToIdle_DMA(&huart3, rx_buffer3, BUF_SIZE);
	/* USER CODE END 2 */
	...
	while (1) {
		...
		if (HAL_GetTick() - cli_tick > 10) {
			cli_tick = HAL_GetTick();
			scheduler_task();
		}
	}
	/* USER CODE END 3 */
