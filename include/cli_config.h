...
/* 条件编译：用户/环境变量/变量导出系统 */
#define CLI_ENABLE_USER 0
#define CLI_ENABLE_ENV 0
#define CLI_ENABLE_VAR 0

/* 条件编译：高级 Tab 补全（选项补全/候选值/高亮循环）
 * 关闭后仅保留命令名前缀匹配 + 列表打印 */
#define CLI_ENABLE_ADVANCED_COMPLETION 0

/* 条件编译：帮助系统（--help 自动生成与用法提示） */
#define CLI_ENABLE_HELP 0

/* 条件编译：命令链（支持 && 分隔的多命令链式执行） */
#define CLI_ENABLE_CMD_CHAIN 0

/* 条件编译：自动运行（CLI_AUTO_CMD 注册的开机自动执行命令） */
#define CLI_ENABLE_AUTO_RUN 0
