...
框架在 Tab 补全的终端输出上做了以下精细处理，保证串口终端的显示始终干净：

- **无死行残留**：列出候选后，光标通过 `\033[1A` 直接上移回到原命令行位置进行覆盖重绘，不会在候选列表下方再残留一条重复的原命令行。
- **连续按 Tab 不堆叠**：每次列出候选前，先用 `\033[2K` 清除该行旧内容，再输出新候选。因此连续按 `Tab` 不会导致新旧候选列表层层叠加。
- **回车后无残留**：按下回车执行命令前，框架会在 `\r\n` 换行后紧跟 `\033[K` 清行，彻底消除可能残留在命令行下方的候选列表，避免命令输出与旧候选内容拼接在一起。
