...
> 实际上，熟悉C语言的开发者，仅需参考tests文件夹里面的测试用例即可大致明白添加命令的流程，这些测试用例都很简洁易懂，
...
这是命令的**响应函数**（也叫 handler）。当用户在终端输入 `log` 并带上对应的选项、按下回车后，只要命令解析成功，框架就会调用这个函数。参数 `_args` 指向的就是上一步定义的 `log_args` 结构体，里面已经填好了用户输入的值。 框架保证，参数结构体_args的缺省值一定是0,这对于非必需选项对应的参数结构体成员很有用！
