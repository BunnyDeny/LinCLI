...
> 🪶 **轻到离谱**：即使开启**全部功能**（用户管理、环境变量、变量导出、高级 Tab 补全、帮助系统、命令链、自动运行），LinCLI 在 ARM Cortex-M4 上仅占 **~24 KB Flash**（`arm-none-eabi-gcc -Os`，无 LTO）。通过条件编译按需关闭模块，最小可裁剪至 **~13 KB**。详见下方 [条件编译与内核裁剪](#条件编译与内核裁剪) 章节。
...
| 宏 | 功能 | 默认 | 关闭后节省 |
|------|------|:----:|:---------:|
| `CLI_ENABLE_ADVANCED_COMPLETION` | 高级 Tab 补全（选项补全、候选值、高亮循环） | `1` | **~4.9 KB** |
...
| **全开（默认）** | **~24.1 KB** |
| 关高级补全 | ~19.2 KB |
| 关高级补全 + 环境变量 + 变量导出 | ~15.4 KB |
| 关上述三项 + 帮助 + 命令链 + 自动运行 | ~14.3 KB |
| **全部关闭（仅核心骨架）** | **~13.1 KB** |
...
- 🖨️ **[内存转储工具 hexdump](docs/hexdump.md)** — 轻量级内存 dump 组件，支持十六进制 + ASCII 双列显示、🔵 蓝色地址列、🔷 青色 ASCII 列、自动表头与分隔线。支持 `--config` 模式配置地址范围保护，以及 `hexdump_default_read` 弱符号自定义读取。包含 Linux 虚拟地址禁区（0~4KB 军事禁区）与段错误原理详解。

---

## ⚖️ 选型对比 <a id="选型对比"></a>

在决定采用 LinCLI 之前，你可能也在考察其他嵌入式 CLI 框架。以下文档从设计哲学、功能矩阵、典型场景等维度，对 **LinCLI** 与国产知名框架 **letter-shell** 进行了客观对比，帮助你和你的团队做出合适的选择：

- 📊 **[LinCLI vs letter-shell：选型指南](docs/lincli_vs_letter_shell.md)** — 涵盖两者核心差异、功能矩阵、五类典型场景选型建议以及常见误区澄清。
