...
在开始之前，你需要先搭建编译环境并了解项目的基本结构。

- **[环境搭建与编译运行](Documentation/setup.md)** — 涵盖 GCC、CMake 安装，以及 PC 端编译、运行、退出方式。
- **[项目结构与核心机制](Documentation/architecture.md)** — 介绍 `cli/`、`lib/`、`init/`、`tests/` 各目录职责，以及链接脚本段自动收集命令的核心原理。
...
1. **不能同时开和关** — LED 只有亮/灭两种状态，`-o/--on` 和 `-f/--off` 必须**互斥**；
...
注意这里**没有**用 `else` 把 `on` 和 `off` 写成互斥分支，而是直接暴力判断 `if (args->on)` 和 `if (args->off)`。因为框架在调用 handler 之前，已经通过 `--on` 的 `conflicts="off"` 保证了两者不可能同时为 `true`。你不需要在 handler 里重复做互斥检查，只需要各自处理自己的业务逻辑即可。同理，`depends` 校验也已经确保了：如果 `--on` 出现了，则 `--brightness` 一定也出现了。

### 第 3 步：用 `CLI_COMMAND` 注册命令

```c
CLI_COMMAND(led, "led", "Control LED", led_handler, (struct led_args *)0,
    OPTION('o', "on",  BOOL, "Turn LED on",  struct led_args, on,  0, "brightness", "off", false),
    OPTION('f', "off", BOOL, "Turn LED off", struct led_args, off, 0, NULL,         "on",  false),
...
| 8 | `"brightness"` | **依赖列表**。空格分隔的多个长选项名字符串。表示：只有当列表中所有选项都出现时，本选项才是合法的。不需要依赖时填 `NULL`。这里 `--on` 依赖 `--brightness`，意味着用户开灯时必须同时给出亮度值。 |
...
```bash
lin@linCli> led -o
[ERR] option -o/--on depends on brightness but not provided
...
```bash
lin@linCli> led -o -f
[ERR] option -o/--on conflicts with off, cannot be used together
[ERR] command parsing failed: led
...
lin@linCli>
```
...
lin@linCli> led --help
 ...
 option      :
  -f, --off              Turn LED off
  -o, --on               Turn LED on [depends:brightness] [conflicts:off]
