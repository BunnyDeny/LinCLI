...
LinCLI 目前还有一些正在完善或尚未支持的特性，如果你在使用过程中遇到以下情况，属于已知行为：

1. **INT_ARRAY 已改为静态内存分配**
   `INT_ARRAY` 类型原先使用 `calloc` 动态分配数组内存，现已改为直接从命令的 `arg_buf` 剩余空间中静态分配，**不再依赖堆内存**。只要 `arg_buf_size` 大于 `arg_struct_size` 且余量足够容纳 `max_args * sizeof(int)`，即可正常工作。如果缓冲区不足，框架会在解析时报错提示。

2. **Token 解析不支持引号与转义**
   当前的 `tokenize` 按简单的空白字符分割，因此像 `ts -m "hello world"` 这种带引号的字符串并不会被识别为单个参数。需要输入空格字符串时，可以直接写 `ts -m hello world`（LinCLI 会自动把后续无 `-` 前缀的 token 拼接起来），但暂不支持 `"` 和 `\` 转义。

3. **错误码粒度较粗**
   目前解析失败统一返回 `-1`，后续可能会引入更细粒度的错误码（如缺少参数、类型不匹配、互斥冲突等），方便上层做更精确的错误处理。
