...
- **`CLI_USER`** 解决的是**"谁能用"**的问题。部署者（或产品定义者）决定哪些已上架的命令对某个用户角色可见。这是**产品安全型**设计的收口。
...
[INFO] already logged in as 'admin'
```
...
当用户输入一个未授权的命令并回车时，框架在 `cmd_parse_prepare` 阶段直接拦截：
...
当用户按 `Tab` 键补全命令名时，框架只会列出该用户**有权使用**的命令：
...
这意味着**普通用户甚至不知道系统里存在哪些未授权命令**——攻击面被彻底隐藏。

---

## 全局当前用户指针

框架暴露一个全局指针 `current_user`，任何代码都可以读取：

```c
extern const cli_user_t *current_user;
```

默认值为 `admin`。命令提示符 `cli_prompt_print` 使用 `current_user->username` 动态显示当前登录用户，切换用户后提示符会即时更新。

---

## 开发者友好型设计的体现

回顾整个用户管理子系统的实现，它的侵入性极低：

- 零删除原有代码
- `cmd_dispose.c` 仅插入 5 行权限判断
- `cli_completion.c` 仅在 6 个遍历循环中各加 2 行过滤
- 全部通过链接脚本段收集和统一遍历宏完成扩展

这正是 LinCLI 架构优秀的体现：**开闭原则**（对扩展开放、对修改关闭）在嵌入式 C 中的实践。开发者只需要关心"我要注册什么命令、什么用户"，框架自动完成收集、解析、补全、权限控制等全部工作。
