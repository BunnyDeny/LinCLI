...
裸机 MCU 只有一个 `while(1)` 主循环，所有业务都挤在里面跑。传统 `CLI_COMMAND` 注册的命令使用单一 handler，框架会**同步阻塞式**调用它——handler 不返回，调度器就永远等在那里。这意味着：

- handler 里写 `delay_ms(500)` → 整个 MCU 冻结 500ms，其他外设全停摆
- handler 里 `while(!flag)` 空转等待 → CPU 被吃满，ADC 不采样、电机不控制、看门狗不喂
- 即使命令本身只需要"逐步完成"（如电机缓慢升速），也必须一次性阻塞到底

**这不是"CLI 能不能响应新输入"的问题，而是整个单片机只剩这一个 while(1)，被命令独占后其他事情全干不了。**

LinCLI 提供 `CLI_COMMAND_ASYNC` 宏，把命令响应拆成 `entry` → `task` → `exit` 三阶段。`task` 每次只执行一个时间片（对应调度器的一次轮询），做完就主动交出 CPU，调度器可以去干别的：读传感器、控其他电机、喂看门狗、处理用户自己的业务逻辑。下次轮询再回来继续执行。这样，**一个需要 5 秒才能完成的命令，不会吃掉这 5 秒内的全部 CPU，而是和其他任务分时复用这一个 while(1)**。
...
| 返回值 | 含义 | 调度器行为 |
|--------|------|-----------|
| `CLI_CONTINUE` (`1`) | 本次执行完毕，但命令不结束 | 保持 `scheduler_cmd_run` 状态，10ms 后再次调用 `task` |
| `0` | 执行成功，命令结束 | 离开 `scheduler_cmd_run`，执行 `exit`，回到提示符/下一命令 |
| `< 0` | 执行失败，命令结束 | 同上，调度器会在 `exit` 后自动打印 `[ERR] command 'xxx' execution failed, return value: N` |
| `> 1` | 扩展语义，视为成功结束 | 同 `0` |
...
`motor_task` 被调用了 6 次（每次调度周期 10ms，实际间隔取决于你的 `scheduler_task()` 调用频率）。在这 50ms 里，调度器每次执行完 motor_task 后都会回到主循环，CPU 可以去干别的——读其他传感器、控制其他外设、执行你自己的业务代码。
...
| 调度器阻塞 | 是（handler 不返回就卡住） | 否（每次只执行一个时间片） |
...
异步命令的执行被封装在调度器的 `scheduler_cmd_run` 状态中：
...
3. **长时间阻塞仍然不好**：即使使用了异步接口，单次 `task` 的执行时间也应该尽量短（建议 < 1ms）。如果 `task` 里做了 `vTaskDelay(100)` 或大量浮点运算，仍然会推迟调度器的下一次轮询。

4. **命令链中的异步命令**：`cmd1 && cmd2` 中如果 `cmd1` 是异步命令，它会完整执行完（`entry` → 多次 `task` → `exit`）之后，才会开始执行 `cmd2`。这是正确的行为——命令链的语义是"前一个成功后才执行后一个"。

5. **自启动命令支持异步**：`cli_auto_cmds[]` 中的命令可以是异步命令，调度器会正确等待它完成后再执行下一个自启动命令。
