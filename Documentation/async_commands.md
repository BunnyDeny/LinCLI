...
传统 `CLI_COMMAND` 注册的命令使用单一 handler，框架会**同步阻塞式**调用它——handler 不返回，调度器就永远等在那里。如果 handler 内部用 `while(1)` 轮询等待某个标志位，整个 CLI 就会卡死，用户无法再输入任何命令。

LinCLI 提供 `CLI_COMMAND_ASYNC` 宏，把命令响应拆成 `entry` → `task` → `exit` 三阶段。`task` 每次只执行一次（对应调度器的一次轮询周期），通过返回值控制是继续执行还是结束。这样命令可以在多次调度周期中"分片"执行，调度器始终保持响应。
...
`motor_task` 被调用了 6 次（每次调度周期 10ms，实际间隔取决于你的 `scheduler_task()` 调用频率），但调度器在这期间始终可以响应新输入。
