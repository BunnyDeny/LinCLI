...
> ⚠️ **类型匹配要求**：`CLI_VAR` 的 `TYPE` 必须与 C 变量的实际类型严格对应。`STRING` 类型要求变量为 `char[]` 数组（如 `char buf[32]`），不能使用 `char *` 指针——因为框架内部通过 `sizeof(symbol)` 获取变量大小来做边界检查，数组能返回正确的长度，而指针只能返回指针本身的大小（如 8 字节）。
...
> ⚠️ **命名约定**：`CLI_VAR_TYPE` 的第一个参数和 `CLI_VAR_CUSTOM` 的第三个参数（字符串）**必须一致**。框架在运行时通过字符串比较来查找对应的类型操作回调。
...
> ⚠️ 注意：`g_ki`、`g_target` 是只读变量，因此不会出现在 `-w` 的补全列表中。
