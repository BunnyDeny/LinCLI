...
**你不需要关心 Python 脚本怎么解析，只需要在 C 代码里按上面的格式打印数据。**
...
这套桥接协议的核心是 **`CLI_COMMAND_ASYNC`** 异步非阻塞命令机制与 **`$SCOPE_START` + `\r` 数据帧协议**：

- ✅ 用 `CLI_COMMAND_ASYNC` 把周期性采样任务拆成 `entry/task` 两阶段，不阻塞 CLI 主循环
- ✅ 用 `$SCOPE_START` 握手信号实现 MCU 与 PC 上位机的会话同步
- ✅ 用 `\r` 前缀输出 CSV 数据，既能在终端上实现单行刷新效果，又能被 Python 脚本按帧解析
