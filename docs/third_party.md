...
> ⚠️ **执行上下文提醒** 🔔：workqueue 的回调运行在 **LinCLI 调度器的任务上下文**中（非中断上下文），但调度器是单线程顺序执行的，work 回调执行期间会**阻塞正常的 CLI 响应**，因此不宜占用过长时间。💡
...
再次提醒 ⚠️：workqueue 回调运行在 LinCLI 调度器线程中，执行期间会**阻塞 CLI 响应**，不宜占用过长时间。😅
