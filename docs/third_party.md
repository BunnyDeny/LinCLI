...
> 🎯 本文介绍 LinCLI 内置的第三方工作队列组件，帮助你在 CLI 线程之外安全地投递异步任务，避免阻塞终端输入～ ✨
...
这样可以把中断里不宜做的复杂逻辑（如链表操作、打印输出、状态机推进）安全地推迟到调度器里处理，避免在中断中停留过久。⚠️ 但请注意：workqueue 回调**仍然运行在 LinCLI 调度器线程中**，执行期间会阻塞命令解析、按键输入等 CLI 功能。
...
> ⚠️ **执行上下文提醒** 🔔：workqueue 的回调运行在 **LinCLI 调度器的任务上下文**中（非中断上下文）。由于调度器是单线程顺序执行的，work 回调执行期间**整个调度器会被阻塞** 😱，命令解析、按键输入、Tab 补全等 CLI 功能全部暂停。因此 work 回调中应只做**轻量级操作**（如设置标志位、翻转 GPIO、启动 DMA）。真正的耗时工作请使用 **异步非阻塞命令**（`CLI_COMMAND_ASYNC`）拆分执行。💡
...
再次提醒 ⚠️：workqueue 回调运行在 LinCLI 调度器线程中，执行期间**直接阻塞整个调度器** 🚫，导致：
...
**workqueue 的正确用法** ✅：
- **中断里**检测到事件 → `schedule_work()` 投递任务 → **调度器里**执行轻量级收尾（如更新状态、清标志、启动下一次 DMA）
- 这样避免在中断里做链表/打印等复杂操作，同时也不会让调度器卡死
- 真正的耗时任务（电机控制、Flash 擦写、传感器等待）请使用 **`CLI_COMMAND_ASYNC`** 拆分为多周期执行
- **千万不要**把 workqueue 当成"后台线程"来跑重活——LinCLI 调度器只有一个线程 😅
