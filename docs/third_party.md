...
**workqueue** 🚀 是 LinCLI 从 Linux 内核工作队列机制移植而来的轻量级实现，专为 MCU / 裸机环境裁剪。它提供以下能力：

| 特性 | 说明 |
|------|------|
| 📦 零依赖 | 仅依赖同仓库的 `list` 组件（链表），无外部库 |
| 🚀 异步执行 | 将耗时操作投递到后台 tick 循环中执行，不阻塞 CLI |
| ⏱️ 延迟调度 | 支持指定 ticks 后执行，链表按过期时间自动升序排列 |
| 🔄 取消任务 | 提供 `cancel_work_sync()` / `cancel_delayed_work()` 安全撤销 |
| 🧹 flush 能力 | `flush_workqueue()` 等待队列中所有任务完成 |

> ⚠️ **执行上下文提醒** 🔔：workqueue 的回调运行在 **LinCLI 调度器的任务上下文**中（非中断上下文），但调度器是单线程顺序执行的。如果某个 work 函数执行耗时过长，会**阻塞整个调度器** 😱，导致命令解析、按键输入、Tab 补全等 CLI 功能出现卡顿或无响应。因此，work 回调中应只做轻量级操作（如设置标志位、翻转 GPIO、启动 DMA），真正的耗时工作应拆分为**异步非阻塞命令**（`CLI_COMMAND_ASYNC`）或在独立中断/DMA 中完成。💡
...
再次提醒 ⚠️：workqueue 回调运行在 LinCLI 调度器线程中，虽然比中断上下文安全，但如果执行耗时操作（如大量计算、阻塞等待、Flash 全片擦除），会**直接阻塞整个调度器** 🚫，导致：
...
**推荐做法** ✅：
- work 回调只做**轻量级操作**（翻转 GPIO、设置标志位、启动 DMA）
- 真正的耗时任务使用 **`CLI_COMMAND_ASYNC`** 拆分为多周期执行
- 或在中断/DMA 完成回调中再 `schedule_work()` 通知结果
