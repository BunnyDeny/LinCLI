# LinCLI vs letter-shell：选型指南

> 本文档旨在帮助开发者在 **LinCLI** 与 **letter-shell** 之间做出适合自身项目的选择。两款框架均为国产优秀的嵌入式 C 语言 CLI 解决方案，采用 MIT 许可证，但在设计哲学上走了两条截然不同的道路。
>
...
| 项目 | 一句话描述 |
|------|-----------|
| **LinCLI** | 一个面向产品的**微型 GNU 风格命令行解析器**，强调选项约束、自动生成帮助、异步调度 |
...
### 场景 B：研发调试 —— 选 letter-shell

**特征**：
- 开发阶段需要快速验证硬件功能
- 团队内部使用，不面向外部客户
- 已有大量 C 函数（`read_adc()`、`set_pwm()`），希望快速暴露到串口
- 不想为每个函数写结构体和 OPTION 宏

**理由**：
- `SHELL_EXPORT_CMD(func, func, "test")` 三秒导出一个函数
- 直接调用 `func(100, "hello")` 的体验比定义 `--num 100 --msg hello` 更直觉
- `exec 0x08028621` 调试未导出函数是底层开发的救命稻草
- 快捷键绑定（如 F1 触发复位、F2 打印状态）在调试时效率极高

### 场景 C：RTOS/Linux 嵌入式子系统 —— 选 LinCLI
...
- LinCLI 最小化约 ~11.5KB，对于极端资源场景可能仍显奢侈
...
### 误区 1："LinCLI 比 letter-shell 大很多，所以资源受限应该选 letter-shell"
...
### 误区 2："letter-shell 的函数签名可以替代 LinCLI 的选项解析"
...
| 你的核心诉求 | 推荐选择 |
|-------------|---------|
| 快速导出 C 函数用于调试 | **letter-shell** |
| 构建面向客户/产线的专业 CLI | **LinCLI** |
| 需要异步非阻塞命令执行 | **LinCLI** |
| 需要选项依赖/互斥/自动帮助 | **LinCLI** |
| 需要网络远程访问 | **pshell**（第三方） |
| 资源极端受限（< 8KB） | **Google esh** 或 **microrl** |
| 两个都想要 | **letter-shell 调试用 + LinCLI 产品用** |
